@extends('back.back-master')

@section('judul_halaman', 'Update Unit')

@section('back-konten')
<a class="tombol" style="float:left" href="/unit">Kembali</a>
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-body">
                    @foreach($data_unit as $unit)
                    <form method="POST" action="/update-unit">
                        @csrf

                        <div class="row mb-3">
                            <label for="unit_id" class="col-md-4 col-form-label text-md-end">{{ __('UnitID') }}</label>

                            <div class="col-md-6">
                                <input id="unit_id" type="text" class="form-control" name="unit_id" value="{{ $unit->unit_id }}" required autocomplete="unit_id" autofocus readonly>
                            </div>
                        </div>

                        <div class="row mb-3">
                            <label for="unit_nama" class="col-md-4 col-form-label text-md-end">{{ __('Nama Unit') }}</label>

                            <div class="col-md-6">
                                <input id="unit_nama" type="text" class="form-control" name="unit_nama" value="{{ $unit->unit_nama }}" autocomplete="unit_nama">
                            </div>
                        </div>

                        <div class="row mb-0">
                            <div class="col-md-6 offset-md-4">
                                <button type="submit" class="btn btn-primary">
                                    {{ __('Submit') }}
                                </button>
                            </div>
                        </div>
                    </form>
                    @endforeach
                </div>
            </div>
        </div>
    </div>
</div>
@endsection